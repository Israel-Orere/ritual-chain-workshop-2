...
describe("RitualPredict local workshop extension", async function () {
  ...
  async function deploy() {
    // 1000 ms makes duration -> block conversion intuitive on the local chain.
  ...
  it("creates a market locally and stores an immutable resolution rule", async () => {
    ...
    assert.equal(created[0], 1n);
    assert.equal(created[1].toLowerCase(), creator.account.address.toLowerCase());
    assert.equal(created[2], params.question);
    assert.equal(created[5], params.target);
    assert.equal(created[6], params.comparator);
  });
  ...
  it("quotes the pari-mutuel payout before a bet is submitted", async () => {
    ...
    const quote = await market.read.potentialPayout([
      1n,
  ...
  it("previews all comparator rules deterministically", async () => {
    const market = await deploy();
    assert.equal(await market.read.previewOutcome([11n, 10n, 0]), 1); // GT -> YES
    assert.equal(await market.read.previewOutcome([10n, 10n, 1]), 1); // GTE -> YES
    assert.equal(await market.read.previewOutcome([10n, 10n, 2]), 2); // LT -> NO
    assert.equal(await market.read.previewOutcome([10n, 10n, 3]), 1); // LTE -> YES
  });
  ...
  it("rejects empty rules, too-short windows and zero-value bets", async () => {
    ...
    await assert.rejects(
      market.write.createMarket([{ ...params, question: "" }], { account: creator.account }),
    );
    await assert.rejects(
      market.write.createMarket([{ ...params, bettingSeconds: 1n }], { account: creator.account }),
    );
  ...
  it("closes betting according to block number rather than timestamp", async () => {
    ...
    assert.equal(closed[12], 1); // Closed view state
    await assert.rejects(
      market.write.bet([1n, true], { account: alice.account, value: parseEther("1") }),
    );
  });
});
