...
/**
 * @title RitualPredict
 * @notice Self-resolving binary prediction markets powered by Ritual Chain.
 * @dev Markets use block-number deadlines, Ritual Scheduler callbacks, a TEE HTTP
 *      oracle read and jq extraction. Failed oracle reads are retried and never
...
contract RitualPredict {
    ...
    function createMarket(NewMarket calldata p) external returns (uint256 marketId) {
        ...
        uint256 resolve_ = close + _secondsToBlocks(p.resolveDelaySeconds);
        if (resolve_ > type(uint32).max) revert BadDuration();

        marketId = ++marketCount;
        uint64 closeBlock = uint64(close);
        uint64 resolveBlock = uint64(resolve_);
    ...
    function onScheduledResolve(uint256 executionIndex, uint256 marketId) external {
        ...
        if (block.number < m.closeBlock) return;
        ...
        if (winningPool == 0) {
            _cancelRemaining(m.scheduleId);
            _invalidate(m, marketId, "empty winning side");
            return;
        }
    ...
    function getMarkets() external view returns (Market[] memory all) {
        ...
        for (uint256 i = 0; i < total; i++) all[i] = getMarket(total - i);
    }

    function stakesOf(uint256 marketId, address account) external view returns (uint256 yes, uint256 no, bool alreadySettled, uint256 claimable) {
        Market storage m = _market(marketId);
        yes = yesStake[marketId][account];
        no = noStake[marketId][account];
    ...
    // Extension: expose pool-implied probability for frontends without off-chain math.
    function impliedOdds(uint256 marketId) external view returns (uint256 yesBps, uint256 noBps) {
        Market storage m = _market(marketId);
        uint256 pool = m.totalYes + m.totalNo;
        if (pool == 0) return (5_000, 5_000);
        yesBps = (m.totalYes * 10_000) / pool;
        noBps = 10_000 - yesBps;
    }

    // Extension: quote a hypothetical payout before the user submits a bet.
    function potentialPayout(uint256 marketId, address account, bool isYes, uint256 amount) external view returns (uint256) {
        Market storage m = _market(marketId);
        uint256 yes = m.totalYes + (isYes ? amount : 0);
        uint256 no = m.totalNo + (isYes ? 0 : amount);
        uint256 winningPool = isYes ? yes : no;
        if (winningPool == 0) return 0;
        uint256 stake = (isYes ? yesStake[marketId][account] : noStake[marketId][account]) + amount;
        return (stake * (yes + no)) / winningPool;
    }

    // Extension: useful for UIs and tests that want to explain a rule before resolution.
    function previewOutcome(uint256 observed, uint256 target, Comparator comparator) external pure returns (Outcome) {
        return _compare(observed, target, comparator) ? Outcome.Yes : Outcome.No;
    }
    ...
    function _readOracle(Market storage m, address executor) private returns (bool ok, uint256 value, string memory reason) {
        ...
        (bool callOk, bytes memory raw) = RitualChain.HTTP_PRECOMPILE.call(encoded);
        if (!callOk) return (false, 0, "http precompile revert");
        if (raw.length == 0) return (false, 0, "empty http response");

        uint16 status;
        bytes memory body;
        string memory errorMessage;
        try this.decodeHttpResponse(raw) returns (uint16 s, bytes memory b, string memory e) {
        ...
        if (status != 200) return (false, 0, "http status not 200");
    ...
    function _pickExecutor(uint256 marketId, uint256 executionIndex) private view returns (address) {
        uint256 seed = uint256(keccak256(abi.encode(marketId, executionIndex, block.number)));
    ...
    function _scheduleResolution(uint256 marketId, uint64 resolveBlock) private returns (uint256 callId) {
        // Hardhat has no Ritual system contracts. Returning a deterministic local id
        // keeps market creation testable locally without changing Ritual Chain behavior.
        if (block.chainid == 31337) return marketId;
    ...
    function _cancelRemaining(uint256 scheduleId) private {
        if (scheduleId == 0 || block.chainid == 31337) return;
        try IScheduler(RitualChain.SCHEDULER).cancel(scheduleId) {} catch {}
    }
    ...
    function _secondsToBlocks(uint256 seconds_) private view returns (uint256 blocks) {
